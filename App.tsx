...
import { 
  ...
  LogOut, Shuffle, Lock, User, Key, RefreshCw, Database
} from 'lucide-react';
import { INITIAL_QUESTIONS, INITIAL_EXAMS, INITIAL_STUDENTS } from './data/initialData';
...
const App: React.FC = () => {
  ...
  const [createdExams, setCreatedExams] = useState<ExamConfig[]>(() => {
      const saved = localStorage.getItem('mm_exams');
      return saved ? JSON.parse(saved) : INITIAL_EXAMS;
  });

  const [studentAccounts, setStudentAccounts] = useState<StudentAccount[]>(() => {
      const saved = localStorage.getItem('mm_students');
      return saved ? JSON.parse(saved) : INITIAL_STUDENTS;
  });

  const [submissions, setSubmissions] = useState<StudentSubmission[]>(() => {
     try {
       const saved = localStorage.getItem('mm_submissions');
       return saved ? JSON.parse(saved) : [];
     } catch(e) { return []; }
  });

  // --- EFFECT TO SAVE TO LOCAL STORAGE WHEN CHANGED ---
  useEffect(() => localStorage.setItem('mm_questions', JSON.stringify(reviewQuestions)), [reviewQuestions]);
  useEffect(() => localStorage.setItem('mm_exams', JSON.stringify(createdExams)), [createdExams]);
  useEffect(() => localStorage.setItem('mm_students', JSON.stringify(studentAccounts)), [studentAccounts]);
  useEffect(() => localStorage.setItem('mm_submissions', JSON.stringify(submissions)), [submissions]);
  ...
  // Handle manual "Factory Reset" Button
  const handleFactoryReset = () => {
      if(confirm("Bạn có chắc muốn khôi phục dữ liệu gốc? Mọi đề thi và kết quả đã lưu sẽ bị xóa.")) {
          setReviewQuestions(INITIAL_QUESTIONS);
          setCreatedExams(INITIAL_EXAMS);
          setStudentAccounts(INITIAL_STUDENTS);
          setSubmissions([]);
  ...
  const handleDeleteExam = (id: string) => { 
      setCreatedExams(prev => prev.filter(e => e.id !== id)); 
  };
  ...
  const handleStartExam = (variant: number) => {
    ...
    // --- LOGIC: Always prefer specificQuestions if available ---
    if (pendingExamConfig.specificQuestions && pendingExamConfig.specificQuestions.length > 0) {
        selectedQuestions = [...pendingExamConfig.specificQuestions];
    } 
    // Fallback logic not really needed if exams are created correctly
    else if (pendingExamConfig.sections && pendingExamConfig.sections.length > 0) {
       pendingExamConfig.sections.forEach(sec => {
          let pool = reviewQuestions; // Use the main pool
    ...
    if (selectedQuestions.length === 0) {
      alert(`Đề thi này không có câu hỏi nào (Hoặc dữ liệu nguồn đã bị xóa).`);
      return;
    }
      ...
      <div className="math-pattern min-h-screen flex flex-col font-sans text-slate-800">
         <header className="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 sticky top-0 z-50 shadow-sm">
            <div className="flex items-center gap-2">
               ...
               <div className="flex flex-col"><span className="font-bold text-slate-800 hidden sm:block leading-tight">Học toán cùng T.Bình</span>{studentInfo && <span className="text-xs text-slate-500 font-medium">{studentInfo.name} - {studentInfo.class}</span>}</div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex flex-col">
      <header className="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 sm:px-6 sticky top-0 z-30 shadow-sm">
        ...
        <div className="flex items-center gap-2">
            <nav className="hidden md:flex items-center gap-1 bg-slate-100 p-1 rounded-xl">
               ...
               <button type="button" onClick={() => setActiveTab('exam')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'exam' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}><div className="flex items-center gap-2"><FileText className="w-4 h-4" /> Làm Bài Thi</div></button>
            </nav>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex justify-around p-2 z-40">
         ...
         <button type="button" onClick={() => setActiveTab('exam')} className={`flex flex-col items-center gap-1 p-2 rounded-lg w-full ${activeTab === 'exam' ? 'text-indigo-600 bg-indigo-50' : 'text-slate-400'}`}><FileText className="w-5 h-5" /> <span className="text-[10px] font-bold">Thi</span></button>
      </nav>
        ...
        {activeTab === 'admin' && (
            <div className="animate-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="mt-8 pt-6 border-t border-slate-200 text-center">
                    <button onClick={handleFactoryReset} className="text-xs text-red-400 hover:text-red-600 underline flex items-center justify-center gap-2 mx-auto">
                        <Database className="w-3 h-3"/> Khôi phục dữ liệu gốc (Factory Reset)
                    </button>
                    <p className="text-[10px] text-slate-400 mt-2">Dữ liệu được lưu trực tiếp trên trình duyệt này.</p>
                </div>
            </div>
        )}
      </main>
